...
if __name__ == "__main__":
    ...
    stack_state = np.concatenate((one_state,one_state,one_state))
    ...
    igl.load_state_dict(torch.load('./model_save/IGL_stack_imp'))
    ...
    while True:
        for i in range(300):
            ...
            next_=igl(torch.FloatTensor(stack_state).unsqueeze(0))
            ...
            current_subgoal = np.array([0])
            one_state = np.concatenate((one_state, current_subgoal))
            stack_state = np.concatenate((stack_state[24:], one_state))
